...
#Region "【Shared Method】"
    ...
    ''' <returns></returns>
    Public Shared Function Method(ByVal lngTimes As Long, ByVal strMethodName As String, ByVal mtdWrapper As MethodWrapper) As clsBenchMark
    ...
    ''' </summary>
    ''' <returns></returns>
    Private Shared Function EndProc(ByVal sw As Stopwatch, ByVal strMethodName As String) As clsBenchMark
        Console.WriteLine(" - TotalTime : " & sw.ElapsedMilliseconds & "[ms]" & vbCrLf)
        Console.WriteLine(mstrLine & strMethodName & " ---  End  " & mstrLine & vbCrLf)
        Dim Result As New clsBenchMark()
        Result.mstrMethodName = strMethodName
        Result.mdblMilliseconds = sw.ElapsedMilliseconds
        Return Result
    End Function
...
#End If
    ...
    Public Shared Operator <(ByVal left As clsBenchMark, ByVal right As clsBenchMark) As Boolean
